...
public class Tests {
    public static void main(String args[]) {
        ...
        System.out.println(a instanceof Fruits); // true
        System.out.println(p instanceof Fruits); // true
        // 把梨类对象赋值给水果类，其中name值为bigPear,颜色为green
        Fruits bigPear = new Pear("bigPear", "green");
        // 输出当前水果类引用的名称
        System.out.println(bigPear.name);
        // 依次判断当前水果类引用是否为水果类和梨类的子类
        System.out.println(bigPear instanceof Fruits);
        System.out.println(bigPear instanceof Pear);
        // 将当前水果类引用强转为梨类
        Pear pearObj = (Pear) bigPear;
        // 输出当前梨类的颜色
        System.out.println(pearObj.pcolor);
        /********** End **********/
    }
}